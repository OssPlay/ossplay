...
function deriveKeyMaterial(privateKeyPem: string): SshKeyMaterial {
  ...
  const publicKeyLine = `${parsed.type} ${publicSsh.toString('base64')}`;
  ...
  return { privateKeyPem, publicKeyLine, fingerprint: `SHA256:${digest}`, keyType: parsed.type };
}
...
export function parsePastedPrivateKey(pem: string): SshKeyMaterial {
