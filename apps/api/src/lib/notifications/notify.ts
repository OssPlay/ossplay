...
// Called from route handlers (and the background update-check timer, which
// has no Hono Context) after a notify-worthy event — mirrors logAudit/
// logSystemError's shape: a short, deliberate list of call sites, not a
// general-purpose event bus. One multi-row insert, not N calls to a
...
// (invite-accept, project create/delete) rather than repeating the same
// select 3+ times.
...
// newly-observed latestVersion — called from both the background auto-check
// timer (index.ts) and the manual "Check for updates" route
// (instance/instance.overview.ts) so the dedupe logic lives in exactly one
