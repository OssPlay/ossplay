...
- Presigned URLs and signed HLS manifests are minted by the API per project rules (PRD §3), never by the worker or dashboard directly; the local-disk driver instead hands back an API-relative content route, since there's no signing concept for a local file.
