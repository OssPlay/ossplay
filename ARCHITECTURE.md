...
- **`projects.id` is an app-supplied, human-meaningful string, not a random UUID** — a slug derived from the project name, unique across the whole instance (not just the org), used to organize the project's objects in S3 and to key a forthcoming project-scoped API-key feature.
