...
2. **Organization** — scoped to one org via `organizationMembers`. Roles: **`owner`** (full control, can delete the org), **`admin`** (create/manage/delete projects, manage assets, not membership or org deletion), **`member`** (edit existing projects and their rules/assets, but can't create or delete a project).
