...
- **Still one image *name***, `ghcr.io/ossplay/ossplay` — just four tags, not one: `:<version>-api`, `:<version>-dashboard`, `:<version>-worker`, `:<version>-updater` (`latest` too, via `docker/metadata-action`'s `flavor: suffix=-<role>,onlatest=true`). `docker-images.yml` is now a `strategy.matrix: role: [...]` over the same job (one push per role, `target: runner-${{ matrix.role }}`), with the GitHub Release/asset-upload step split into its own job that runs once after the matrix, not once per leg (four concurrent legs racing to create/attach to the same release was the alternative). `infra/docker-compose.yml`'s `api`/`dashboard`/`updater` services each now pull their own role-suffixed tag instead of all three sharing one.
